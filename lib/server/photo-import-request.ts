...
import { responseFromError, type AuthIdentity } from "@/lib/server/platform";

function text(value: FormDataEntryValue | null, maxLength = 240) {
  return typeof value === "string" ? value.trim().slice(0, maxLength) : "";
}

export async function handlePhotoImportRequest(request: Request, identity: AuthIdentity) {
  try {
