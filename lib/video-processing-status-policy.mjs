import { videoProcessingFailure } from "./video-media-processing-policy.mjs";

export const VIDEO_PROCESSING_STEP_STATES = new Set(["done", "active", "pending", "attention"]);

function hasNoUsableAudio(job) {
  return lower(job?.status) === "no_usable_audio" || (lower(job?.status) === "failed" &&
    videoProcessingFailure({ code: job?.errorCode, message: job?.errorMessage }).status === "no_usable_audio");
}
...
export function lessonProcessingStatus(input = {}) {
  ...
  if (hasNoUsableAudio(job)) {
    return {
      code: "needs_attention",
      title: "No usable Coach audio detected",
      explanation: "Audio analysis finished, but there was not enough clear coaching speech for a recap. Add Coach feedback manually.",
      lastUpdatedAt,
      safeFailureCode: "no_usable_audio",
    };
  }
...
function failedProcessingStep(job) {
  if (hasNoUsableAudio(job)) return "transcript";
...
export function lessonProcessingSteps(input = {}) {
  ...
  const audioDone = hasTranscript || hasDraft || failedStep === "transcript" || failedStep === "recap" || ["transcribing", "generating_recap", "ready_for_review", "published"].includes(status.code);
  ...
  return [
    ...
    {
      key: "transcript",
      label: status.safeFailureCode === "no_usable_audio" ? "Not enough coaching speech detected" : "Creating transcript",
...
export function lessonProcessingProgress(input = {}) {
  const steps = lessonProcessingSteps(input);
  const completedCount = steps.filter((step) => step.state === "done").length;
  const activeIndex = steps.findIndex((step) => step.state === "active");
  const attentionIndex = steps.findIndex((step) => step.state === "attention");
  const status = lessonProcessingStatus(input);
  if (status.code === "stored") {
    return {
      completedCount: 0,
      currentLabel: "Audio analysis not started",
      percent: 0,
      state: "pending",
      stepNumber: 1,
  ...
  const currentIndex = attentionIndex >= 0 ? attentionIndex : activeIndex >= 0 ? activeIndex : Math.min(completedCount, steps.length - 1);
  const totalSteps = steps.length || 1;
  const percent = Math.max(0, Math.min(100, Math.round((completedCount / totalSteps) * 100)));
  const currentStep = steps[currentIndex] ?? steps[0] ?? null;
  return {
    completedCount,
    currentLabel: currentStep?.label ?? "Waiting to start",
    percent,
    state: currentStep?.state ?? "pending",
    stepNumber: Math.min(totalSteps, Math.max(1, currentIndex + 1)),
    totalSteps,
  };
}
