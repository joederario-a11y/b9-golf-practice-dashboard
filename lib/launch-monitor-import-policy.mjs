...
export function normalizeLaunchMonitorClubName(value) {
  ...
  return aliases[normalized] ?? (raw || "Unknown Club");
}
...
function validateShots(shots, warnings, blockingIssues) {
  ...
  for (const shot of shots) {
    ...
    if (!shot.club || compactHeader(shot.club) === "unknownclub") {
      blockingIssues.push("One or more rows are missing a club.");
    }
  }
}
...
export function parseLaunchMonitorCsv(text, options = {}) {
  ...
  const shots = rows.slice(1).flatMap((cells, rowIndex) => {
    ...
    const club = normalizeLaunchMonitorClubName(cellValue(cells, clubMapping) || "Unknown Club");
