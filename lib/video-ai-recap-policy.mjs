...
export function buildLessonPublishConfirmation(values) {
  ...
  const body = hasRecap
    ? `Your video and MAI Coach lesson recap were successfully published to ${memberName}. ${memberFirstName} can now review the lesson and practice recommendations from their account.`
    : `The lesson video was published to ${memberName} without a MAI Coach recap. ${memberFirstName} can now watch it from their account.`;
  return {
    body,
    includedLabel: hasRecap ? "Video and MAI Coach lesson recap" : "Video only",
    lessonTitle,
    memberFirstName,
    memberName,
