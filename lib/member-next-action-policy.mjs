...
import {
  activeCoachRelationships as canonicalActiveCoachRelationships,
  coachMatchesActiveContext,
  getActiveCoachContext,
} from "./coach-relationship-policy.mjs";
...
function hasCoachConnection(activity = {}, context = {}) {
  ...
  const connection = instructions.coachConnection && typeof instructions.coachConnection === "object"
    ...
    : {};
  const activeCoachContext = getActiveCoachContext(activeCoachRelationships(context));
  if (!activeCoachContext.hasActiveCoach) return false;
  if (!coachMatchesActiveContext(activity.coachId, activeCoachContext)) return false;
...
function activeCoachRelationships(context = {}) {
  const relationships = Array.isArray(context.activeCoachRelationships)
    ? context.activeCoachRelationships
    : Array.isArray(context.coachRelationships)
      ? context.coachRelationships
      : Array.isArray(context.coaches)
        ? context.coaches
        : [];
  return canonicalActiveCoachRelationships(relationships);
}

function hasActiveCoachRelationship(context = {}) {
  return activeCoachRelationships(context).length > 0 ||
    Number(context.activeCoachCount ?? context.canonicalActiveCoachCount ?? 0) > 0 ||
    Boolean(context.hasActiveCoachRelationship ?? context.hasActiveCoach);
}

function activeMemberActivity(context = {}) {
  return context.activeMemberActivity ?? context.currentActivity ?? null;
}

function hasIndependentSignal(context = {}) {
  const currentActivity = activeMemberActivity(context);
  ...
  const activeMaiActivity = isActivePracticeActivity(currentActivity) && !hasCoachConnection(currentActivity, context);
...
export function getMemberExperienceMode(context = {}) {
  const hasCoach = hasActiveCoachRelationship(context);
...
function buildCoachActions(context, mode) {
  const coaches = activeCoachRelationships(context);
  if (!coaches.length && !hasActiveCoachRelationship(context)) return [];
  const name = coachName(coaches);
  const currentActivity = activeMemberActivity(context);
  const activeCoachContext = getActiveCoachContext(coaches);
  const coachVideos = (context.videos ?? []).filter((video) =>
    coachMatchesActiveContext(video?.coachId, activeCoachContext),
  );
  const latestLesson = context.latestCoachLesson && coachMatchesActiveContext(context.latestCoachLesson.coachId, activeCoachContext)
    ? context.latestCoachLesson
    : selectLatestPlayableCoachLesson(coachVideos);
  const actions = [];

  if (isActivePracticeActivity(currentActivity) && hasCoachConnection(currentActivity, context)) {
    actions.push(action({
      type: "coach_practice",
      ...
      primaryActionUrl: currentActivity.id ? `/practice/${encodeURIComponent(currentActivity.id)}` : "/practice",
...
function buildIndependentActions(context, mode) {
  ...
  if (isActivePracticeActivity(currentActivity) && !hasCoachConnection(currentActivity, context) && currentActivity.activityType !== "challenge") {
    actions.push(action({
      type: "mai_practice",
