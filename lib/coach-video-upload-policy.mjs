...
export function libraryPublishEmailOutcome(values = {}) {
  const beforeEmailStatus = String(values.beforeEmailStatus ?? "").toLowerCase();
  const afterEmailStatus = String(values.afterEmailStatus ?? "").toLowerCase();
  if (beforeEmailStatus === "sent") return "already_sent";
  if (afterEmailStatus === "sent") return "sent";
  if (afterEmailStatus === "failed") return "failed";
  return "not_sent";
}

export function libraryPublishEmailConfirmation(values = {}) {
  const memberName = String(values.memberName ?? "the member").trim() || "the member";
  const outcome = String(values.outcome ?? "not_sent");
  if (outcome === "sent") return `Lesson published to ${memberName} and the email notification was sent.`;
  if (outcome === "failed") return `Lesson published to ${memberName}, but the email notification could not be sent.`;
  if (outcome === "already_sent") return `Lesson is already published to ${memberName}, and the email was already sent. No duplicate email was sent.`;
  return `Lesson published to ${memberName}. No email was sent.`;
}
