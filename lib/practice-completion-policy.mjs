...
export const PRACTICE_EVIDENCE_PRIORITY = [
  "measured",
  "user_confirmed",
  "coach_stated",
  "coach_approved_ai",
...
function clubKeys(value) {
  const raw = text(value).toLowerCase();
  const normalized = normalizeClub(raw);
  const keys = new Set(normalized ? [normalized] : []);
  if (!raw) return keys;
  const firstChoice = raw.split(/\bor\b|\/|\(|,/)[0];
  const firstChoiceNormalized = normalizeClub(firstChoice);
  if (firstChoiceNormalized) keys.add(firstChoiceNormalized);
  if (/\bdriver\b|\b1\s*wood\b/.test(raw)) keys.add("driver");
  if (/\b([2-9])\s*[- ]?\s*iron\b|\b([2-9])i\b/.test(raw)) {
    const match = raw.match(/\b([2-9])\s*[- ]?\s*iron\b|\b([2-9])i\b/);
    keys.add(`${match?.[1] || match?.[2]}iron`);
  }
  if (/\bpitching\s*wedge\b|\bpw\b/.test(raw)) keys.add("pw");
  if (/\bgap\s*wedge\b|\bgw\b/.test(raw)) keys.add("gw");
  if (/\bsand\s*wedge\b|\bsw\b/.test(raw)) keys.add("sw");
  if (/\blob\s*wedge\b|\blw\b/.test(raw)) keys.add("lw");
  if (/\bputter\b/.test(raw)) keys.add("putter");
  return keys;
}
...
function shotMatchesClub(shot, club) {
  const targetKeys = clubKeys(club);
  if (!club || targetKeys.has("anyclub") || targetKeys.has("primaryclub")) return true;
  const shotKeys = clubKeys(shot?.club);
  return [...shotKeys].some((key) => targetKeys.has(key));
}
...
function evidencePriorityForSource(source = "") {
  const normalized = text(source).toLowerCase();
  if (normalized === "measured" || normalized.includes("measured")) return "measured";
  if (normalized === "manual" || normalized.includes("user_confirmed")) return "user_confirmed";
  if (normalized.includes("coach_stated") || normalized === "coach") return "coach_stated";
  if (normalized.includes("coach_approved")) return "coach_approved_ai";
  if (normalized.includes("photo") || normalized.includes("video") || normalized.includes("uploaded")) return "uploaded_observation";
  if (normalized.includes("member") || normalized.includes("student")) return "student_reported";
  if (normalized.includes("ai")) return "ai_inferred";
  return "unknown";
}

function orderedEvidenceSources(outcome = {}, evidenceReferences = {}) {
  const sources = new Set();
  for (const item of Array.isArray(outcome.evidence) ? outcome.evidence : []) {
    sources.add(evidencePriorityForSource(item?.source));
  }
  for (const source of Array.isArray(evidenceReferences.sources) ? evidenceReferences.sources : []) {
    const normalized = text(source).toLowerCase();
    if (normalized === "session" || normalized === "csv") sources.add("measured");
    else if (normalized === "manual" || normalized === "challenge_attempt") sources.add("user_confirmed");
    else if (normalized === "photo" || normalized === "video") sources.add("uploaded_observation");
    else sources.add(evidencePriorityForSource(normalized));
  }
  if (!sources.size) sources.add("unknown");
  return PRACTICE_EVIDENCE_PRIORITY.filter((source) => sources.has(source));
}

export function buildPracticeCompletionIntelligence({
  activity = {},
  attempt = {},
  outcome = {},
  evidenceReferences = {},
  ...
  const hasMeasuredEvidence = outcome.measurementSource === "measured_from_session_data" ||
    orderedEvidenceSources(outcome, evidenceReferences)[0] === "measured";
  const evidenceSources = orderedEvidenceSources(outcome, evidenceReferences);
  const dataTimestamp = new Date().toISOString();
  const completionQuality = outcome.classification === "improved"
    ? "improving"
    : outcome.classification === "declined"
      ? "needs_more_work"
      : outcome.classification === "stable" || outcome.classification === "mixed"
        ? "stable_or_mixed"
        : "insufficient_data";

  return {
    currentPracticePriority: text(activity.focusArea || activity.focus_area, "Practice priority"),
    assignmentCompleted: {
      id: text(activity.id),
      title: text(activity.title, "Practice Plan"),
      source: normalizePracticeAssignment(activity).source,
    },
    attemptId: text(attempt.id),
    resultId: text(resultId) || null,
    completionQuality,
    classification: text(outcome.classification, "insufficient_data"),
    reflection: text(attempt.memberDifficultyLabel || attempt.member_difficulty_label || attempt.reflectionFeeling),
    trainingAidUsed: attempt.trainingAidUsed ?? attempt.training_aid_used ?? null,
    trainingAidHelpfulness: finiteNumber(attempt.trainingAidHelpfulness ?? attempt.training_aid_helpfulness),
    evidenceSubmitted: evidenceSources,
    measuredOutcome: hasMeasuredEvidence ? outcome.evidence ?? [] : [],
    confidence: finiteNumber(outcome.confidence) ?? 0,
    remainingOpportunity: text(outcome.remainingOpportunity) || "Insufficient data",
    recommendedNextAction: text(outcome.recommendedNextAction, "repeat"),
    recommendedReason: text(outcome.recommendedReason, "Insufficient evidence to progress responsibly."),
    dataTimestamp,
    evidenceProvenance: {
      primary: evidenceSources[0] ?? "unknown",
      hierarchy: evidenceSources,
      linkedSessionId: text(evidenceReferences.linkedSessionId) || null,
      linkedChallengeAttemptId: text(evidenceReferences.linkedChallengeAttemptId) || null,
      measurementSource: text(outcome.measurementSource, "member_recorded"),
    },
    trend: hasMeasuredEvidence
      ? completionQuality
      : "insufficient_data",
    trendPolicy: hasMeasuredEvidence
      ? "single_completion_checkpoint_not_long_term_trend"
      : "repeat_with_measured_evidence_required",
  };
}
