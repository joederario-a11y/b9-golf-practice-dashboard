export const VISUAL_ANALYSIS_VERSION = "mai-visual-swing-v1";
...
export function shouldRequestVisualAnalysisProcessing(identity, requested, options = {}) {
  ...
  if (userRole === "coach" || userRole === "admin") return requested === true || options.defaultEnabledInDev === true;
  if (userRole === "member") return requested !== false;
  return false;
}
...
export function canRequestVisualAnalysis(identity, video, assignedMemberIds = []) {
  ...
  const uploadedByRole = role(video.uploadedByRole ?? video.uploaded_by_role);
  if (userRole === "admin") return true;
  if (userRole === "member") return identity.id === memberId && uploadedByRole === "member";
...
export function visualAnalysisInitialVisibility(identity, hasCoachRelationship) {
  const userRole = role(identity?.role);
  if (userRole === "member" && !hasCoachRelationship) return "ready_for_member";
