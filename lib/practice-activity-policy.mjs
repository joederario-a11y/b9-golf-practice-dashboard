import {
  TRAINING_AID_APPROVAL_STATES,
  TRAINING_AID_CONFIDENCE_LEVELS,
  TRAINING_AID_DEFINITIONS,
  TRAINING_AID_NONE_ID,
  normalizeTrainingAidDraft,
} from "./training-aid-policy.mjs";
...
export const practiceActivityJsonSchema = {
  ...
  properties: {
    ...
    trainingAid: {
      ...
      properties: {
        aidId: { type: "string", enum: [TRAINING_AID_NONE_ID, ...TRAINING_AID_DEFINITIONS.map((aid) => aid.id)] },
        name: { type: "string" },
        source: { type: "string", enum: ["coach", "coach_approved_ai", "ai", "none"] },
        confidence: { type: "string", enum: TRAINING_AID_CONFIDENCE_LEVELS },
        evidence: {
          type: "array",
          items: {
            type: "object",
            additionalProperties: false,
            required: ["source", "tendency", "summary"],
            properties: {
              source: { type: "string" },
        ...
        noEquipmentAlternative: { type: "string" },
        approvalState: { type: "string", enum: TRAINING_AID_APPROVAL_STATES },
      },
    },
  },
};
...
export function normalizePracticeActivityOutput(value, requestedActivityType = "drill") {
  ...
  return {
    ...
    trainingAid: normalizeTrainingAidDraft(value.trainingAid),
  };
}
...
export function buildDefaultPracticeActivity({
  ...
  if (safeType === "challenge") {
    return normalizePracticeActivityOutput({
      ...
      trainingAid: {
        ...
        confidence: "low",
        evidence: [],
        whyItFits: "This challenge can be completed with a target line and honest scoring.",
        setupSteps: [],
        safetyNotes: [],
        noEquipmentAlternative: "Use a safe visual target and score the challenge without extra equipment.",
  ...
  return normalizePracticeActivityOutput({
    ...
    trainingAid: {
      ...
      confidence: "low",
      evidence: [],
      whyItFits: "No specialized training aid is required for this starter drill.",
      setupSteps: [],
      safetyNotes: [],
      noEquipmentAlternative: "Use a safe visual target and half-speed rehearsals.",
      approvalState: "not_needed",
    },
  }, "drill");
}
