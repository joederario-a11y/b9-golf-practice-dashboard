...
export const INVITE_STATUSES = new Set(["pending", "sent", "delivered", "opened", "completed", "failed", "expired", "cancelled", "accepted", "active"]);
...
export function deriveSetupStatus(user) {
  ...
  if (user.inviteStatus && user.inviteStatus !== "accepted" && user.inviteStatus !== "active" && user.inviteStatus !== "completed") {
...
export function canManageUserPhoto(identity, targetUser, relationship = {}) {
  if (!identity || !targetUser) return false;
  if (identity.role === "admin") return true;
  if (identity.id === targetUser.id) return true;
  return identity.role === "coach" && targetUser.role === "member" && relationship.isAssignedCoach === true;
}

export function canViewUserPhoto(identity, targetUser, relationship = {}) {
  if (!identity || !targetUser) return false;
  if (identity.role === "admin") return true;
  if (identity.id === targetUser.id) return true;
  if (identity.role === "coach" && targetUser.role === "member") return relationship.isAssignedCoach === true;
  if (identity.role === "member" && targetUser.role === "coach") return relationship.isAssignedMember === true;
  return false;
}
