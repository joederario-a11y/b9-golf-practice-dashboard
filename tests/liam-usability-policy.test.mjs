...
test("coach lesson review uses sticky publish and full-width feedback workflow", () => {
  ...
  assert.match(pageSource, /What to Practice/);
  assert.match(pageSource, /Practice Intelligence/);
  assert.match(pageSource, /Preview and Publish/);
  assert.match(pageSource, /Private Coach support\. Nothing here reaches the Student unless you choose to include it\./);
  ...
  assert.match(pageSource, /Success Goal/);
  ...
  assert.match(pageSource, /Add Coach Drill/);
  assert.match(pageSource, /Add Coach Aid/);
